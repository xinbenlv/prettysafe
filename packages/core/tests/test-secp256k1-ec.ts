...
async function runTests(device: GPUDevice, cases: TestCase[]): Promise<{ passed: number; failed: number }> {
  const fieldWgsl = await Bun.file(new URL("../src/shaders/wgsl/secp256k1-field.wgsl", import.meta.url).pathname).text();
  const keccakWgsl = await Bun.file(new URL("../src/shaders/wgsl/keccak256.wgsl", import.meta.url).pathname).text();
  const ecWgsl = await Bun.file(new URL("../src/shaders/wgsl/secp256k1-ec.wgsl", import.meta.url).pathname).text();
