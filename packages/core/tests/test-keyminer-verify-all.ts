...
async function main() {
  ...
  const shaderDir = join(import.meta.dir, "..", "src", "shaders");
  const shaderSrc = [
    await Bun.file(join(shaderDir, "wgsl", "secp256k1-field.wgsl")).text(),
    await Bun.file(join(shaderDir, "wgsl", "keccak256.wgsl")).text(),
    await Bun.file(join(shaderDir, "wgsl", "secp256k1-ec.wgsl")).text(),
    await Bun.file(join(shaderDir, "keyminer.wgsl")).text(),
  ].join("\n");
